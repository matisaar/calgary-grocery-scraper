"""
No Frills Scraper — same Loblaw Iceberg platform as Superstore.
Uses identical data-testid selectors.
"""
...
CATEGORIES = [
    {"name": "Produce", "url": "https://www.nofrills.ca/food/fruits-vegetables/c/28000"},
    {"name": "Dairy & Eggs", "url": "https://www.nofrills.ca/food/dairy-eggs/c/28001"},
    {"name": "Meat & Seafood", "url": "https://www.nofrills.ca/food/meat-seafood/c/28002"},
    {"name": "Bakery", "url": "https://www.nofrills.ca/food/bakery/c/28003"},
    {"name": "Pantry", "url": "https://www.nofrills.ca/food/pantry/c/28004"},
    {"name": "Frozen", "url": "https://www.nofrills.ca/food/frozen/c/28005"},
    {"name": "Snacks & Candy", "url": "https://www.nofrills.ca/food/snacks-chips/c/28006"},
    {"name": "Beverages", "url": "https://www.nofrills.ca/food/drinks/c/28007"},
]
...
class NoFrillsSpider(scrapy.Spider):
    ...
    async def parse_category(self, response):
        ...
        try:
            ...
            self.logger.info(
                f"[No Frills] {category} p{page_num}: found {len(products)} products"
            )
            ...
            if len(products) > 0 and page_num < self.MAX_PAGES:
                ...
                if has_next:
                    ...
                    self.logger.info(f"[No Frills] {category}: following page {page_num + 1}")
                    yield scrapy.Request(
                        url=next_url,
                        callback=self.parse_category,
                        meta={
            ...
            self.logger.error(f"[No Frills] Error in {category} p{page_num}: {e}")
        finally:
            if page:
                await page.close()
    ...
    def _build_item(self, data, category):
        ...
        loader.add_value("store", "No Frills")
        ...
        if url and not url.startswith("http"):
            url = f"https://www.nofrills.ca{url}"
    ...
    async def errback_close_page(self, failure):
        ...
        if page:
            await page.close()
        self.logger.error(f"[No Frills] Error: {failure}")
